...
export default class Home extends Component {
  ...
  render() {
    let filtro = this.state.dados.filter((dado) => {
      return (
        dado.nome.toLowerCase().indexOf(this.state.searchNome.toLowerCase()) !== -1
      );
    });
    let bairros = this.state.dados.filter((elem, index, arr) => arr.indexOf(elem) === index);
            ...
            <Row form>
              ...
              <Col md={6}>
                <InputGroup>
                  <Input
                    type="text"
                    placeholder="Total por Bairro ..."
                    value={this.state.searchBairro}
                    onChange={this.updateSerachbAirro.bind(this)}
                  />
