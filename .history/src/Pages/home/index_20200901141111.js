...
export default class Home extends Component {
  ...
  buscarPorBairro = (elem) => {
    const response = this.state.dados.filter((bairro) => bairro.bairro === elem);
    const totalEleitores = response.reduce(
      (prevVal, campo) => prevVal + campo.eleitores, 0
    )
    const totalSessoes = response.reduce(
      (prevVal, campo) => prevVal + campo.sessoes, 0
    )    
    return <ModalDetalhes bairro={response} eleitores={totalEleitores} sessoes={totalSessoes} />
  };
